...
const logger = createServiceLogger('publishing-service');
...
app.get('/health', (_req, res) => {
    res.json({ status: 'healthy', service: 'publishing-service' });
});
...
// Start server
app.listen(PORT, () => {
    logger.info(`📚 Publishing Service running on port ${PORT}`);
});
